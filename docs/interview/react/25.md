...
![图片](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/640-20210928205916868)
...
`render`函数里面可以编写`JSX`，转化成`createElement`这种形式，用于生成虚拟`DOM`，最终转化成真实`DOM`，在`React` 中，类组件只要执行了 `setState` 方法，就一定会触发 `render` 函数执行，函数组件使用`useState`更改状态不一定导致重新`render`
...
![图片](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/640-20210928205922931)