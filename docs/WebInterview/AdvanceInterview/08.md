# 「2022」高频前端面试题之React篇（下）



![image-20220210120546648](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210120546648.png)
...
实际上，类组件和函数组件之间，是面向对象和函数式编程这两套不同的设计思想之间的差异。而函数组件更加契合 React 框架的设计理念： ![image-20220210120517561](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210120517561.png) React 组件本身的定位就是函数，一个输入数据、输出 UI 的函数。作为开发者，我们编写的是声明式的代码，而 React 框架的主要工作，就是及时地把声明式的代码转换为命令式的 DOM 操作，把数据层面的描述映射到用户可见的 UI 变化中去。这就意味着从原则上来讲，React 的数据应该总是紧紧地和渲染绑定在一起的，而类组件做不到这一点。**函数组件就真正地将数据和渲染绑定到了一起。函数组件是一个更加匹配其设计理念、也更有利于逻辑拆分与重用的组件表达形式。**
...
实际上，diff 算法探讨的就是虚拟 DOM 树发生变化后，生成 DOM 树更新补丁的方式。它通过对比新旧两株虚拟 DOM 树的变更差异，将更新补丁作用于真实 DOM，以最小成本完成视图更新。 ![image-20220210120454669](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210120454669.png) 具体的流程如下：

- 真实的 DOM 首先会映射为虚拟 DOM；
- 当虚拟 DOM 发生变化后，就会根据差距计算生成 patch，这个 patch 是一个结构化的数据，内容包含了增加、更新、移除等；
- 根据 patch 去更新真实的 DOM，反馈到用户的界面上。

![CgqCHl_qypGAZPuGAADYrK9nkJY878_mh1609406162857.jpg](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210120448001.png) 一个简单的例子：
...
![image-20220210115958428](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210115958428.png)

- 服务端数据请求

![61612599340_.pic.jpg](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210115950758.png) **2)html渲染** 服务端渲染是先向后端服务器请求数据，然后生成完整首屏 html返回给浏览器；而客户端渲染是等js代码下载、加载、解析完成后再请求数据渲染，等待的过程页面是什么都没有的，就是用户看到的白屏。就是服务端渲染不需要等待js代码下载完成并请求数据，就可以返回一个已有完整数据的首屏页面。

- 非ssr html渲染

![image-20220210115941971](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210115941971.png)

- ssr html渲染

![image-20220210115737432](https://cdn.jsdelivr.net/gh/IceRain-mvc/cdn/img/image-20220210115737432.png)
